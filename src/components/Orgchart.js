  ...
  return (
    <table className={addSpace ? 'add-space' : ''}>
      <tbody>
        ...
        {children && children.length > 1 && <tr className='lines'>{lines}</tr>}
...
class OrgChart extends Component {
  ...
  render() {
    ...
    const tables = _.map(groupedData[null], item => {
      return (
        <div className='root-container' key={item.id}>
          {getTR(
            item,
            groupedData,
    ...
    return <div className='org-chart-container'>{tables}</div>;
  }
}
...
class NodeBox extends Component {
  ...
  render() {
    const { node, editMode } = this.state;
      ...
      <ClickOutsideHandler action={editingNodeId !== -1 && editingNodeId === node.id ? changeEditingNodeId : null}>
        {editMode && editable ? (
          ...
          >
            ...
            <div
              onClick={
                (node.ParentId !== null || (node.ParentId === null && rootEditable)) && editable
                  ? this.editNode
                  : undefined
              }
